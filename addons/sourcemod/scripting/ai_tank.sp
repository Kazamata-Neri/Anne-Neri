...
#define TANK_ROCK_AIM_TIME 2.7
#define VISION_UNLOCK_TIME 1.0								// 视角解锁间隔
// CommandAbot
...
// ConVars
ConVar g_hTankBhop, g_hTankThrow, g_hTankBhopMinRange, g_hTankBhopMaxRange, g_hTankThrowDist, g_hTankBlockThrowDist, g_hTankTarget, g_hTankBhopSpeed, g_hTreeDetect, g_hTreeNewTarget, g_hTankAirAngles, g_hTankAttackRange
...
// Ints
int g_iTankTarget, g_iTankAttackRange, g_iTankBhopMinRange, g_iTankBhopMaxRange, g_iTankThrowDist, g_iTankBlockThrowDist, g_iTreeDetect, g_iTreePlayer[MAXPLAYERS + 1] = -1, g_iTreeNewTarget, g_iTankConsumeLimit
...
, g_fRockThrowTime[MAXPLAYERS + 1] = {0.0};
float fLockVisonTime;
// Handles
...
public void OnPluginStart()
{
	...
	g_hTankBhopMinRange = CreateConVar("ai_Tank_Bhop_Min_Range", "75", "Tank距离目标多近停止连跳", FCVAR_NOTIFY, true, 0.0);
	g_hTankBhopMaxRange = CreateConVar("ai_Tank_Bhop_Max_Range", "99999", "Tank距离目标多近开始连跳", FCVAR_NOTIFY, true, 0.0);
	g_hTankThrow = CreateConVar("ai_Tank_Throw", "1", "是否允许Tank投掷石块：0=关闭，1=开启", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_hTankThrowDist = CreateConVar("ai_TankThrowDistance", "99999", "Tank距离目标多近允许投掷石块", FCVAR_NOTIFY, true, 0.0);
	g_hTankBlockThrowDist = CreateConVar("ai_TankBlockThrowDistance", "250", "Tank距离目标多近时阻止投掷石块", FCVAR_NOTIFY, true, 0.0);
	...
	g_hTankAirAngles = CreateConVar("ai_TankAirAngles", "180.0", "Tank在空中的速度向量与到生还者的方向向量夹角大于这个值停止连跳", FCVAR_NOTIFY, true, 0.0, true, 90.0);
	...
	g_hVomitCanInstantAttack = CreateConVar("ai_TankVomitCanInstantAttack", "3", "是否开启固定数量生还者被喷吐后Tank立刻攻击", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	...
	g_hTeleportForwardPercent = CreateConVar("ai_TankTeleportForwardPercent", "100", "Tank开始消耗时，记录此时生还者行进距离x，生还者前压超过x + 这个值时，Tank会传送至生还者处进行压制", FCVAR_NOTIFY, true, 0.0);
	...
	g_hTankConsumeDamagePercent = CreateConVar("ai_TankConsumeDamagePercent", "100", "Tank在消耗过程中只会受到这个百分比的伤害", FCVAR_NOTIFY, true, 0.0, true, 100.0);
	// 2022-4-7新增
	g_hTankForceAttackDistance = CreateConVar("ai_TankForceAttackDistance", "400", "Tank在离最近生还这个距离时即使可以消耗也不会当着生还的面回避生还（强制压制）", FCVAR_NOTIFY, true, 0.0);
	g_hTankAttackIncapped = CreateConVar("ai_TankIncappedCount", "4", "强制压制时，需要拍倒这个数量的生还者才允许继续检测是否可以消耗", FCVAR_NOTIFY, true, 0.0);
	g_hTankConsumeHealthLimit = CreateConVar("ai_TankConsumeHealthLimit", "3000", "Tank在少于这么多血量时不会消耗", FCVAR_NOTIFY, true, 0.0);
	...
	g_hTankConsumeDistance = CreateConVar("ai_TankConsumeDistance", "1500", "Tank消耗找位的位置必须离生还者大于这个距离", FCVAR_NOTIFY, true, 0.0);
	//2022-4-22新增一点好玩的
	...
	g_hSiLimit = FindConVar("z_hunter_limit");
	// HookEvents
	...
	g_hTankBhopMinRange.AddChangeHook(ConVarChanged_Cvars);
	g_hTankBhopMaxRange.AddChangeHook(ConVarChanged_Cvars);
...
void GetCvars()
{
	...
	g_iTankAttackRange = g_hTankAttackRange.IntValue;
	g_iTankBhopMinRange = g_hTankBhopMinRange.IntValue;
	g_iTankBhopMaxRange = g_hTankBhopMaxRange.IntValue;
...
public Action L4D_OnCThrowActivate(int ability)
{
	...
	if (IsAiTank(tankclient) && !g_bTankBhop && GetEntProp(tankclient, Prop_Send, "m_zombieClass") == ZC_TANK && !g_bCanTankConsume[tankclient] && g_bCanTankAttack[tankclient])
	{
...
public Action OnPlayerRunCmd(int tank, int &buttons, int &impulse, float vel[3], float angles[3], int &weapon)
{
	if (IsAiTank(tank))
	{
		float fTankPos[3] = {0.0}, fSpeed[3] = {0.0}, fCurrentSpeed = 0.0, fAngles[3] = {0.0}, fclientEyeAngles[3] = {0.0};
		GetClientAbsOrigin(tank, fTankPos);
		int iSurvivorDistance, iTarget, iFlags, iNearestTarget, iNearestTarget_NotIncap, iInfectedCount = GetInfectedCount(), survivorcount = GetSurvivorCount();
		iSurvivorDistance = GetSurvivorDistance(fTankPos);	iTarget = GetClientAimTarget(tank, true);	iNearestTarget = GetClosestSurvivor(fTankPos);	iNearestTarget_NotIncap = GetClosestSurvivor_NotIncap(fTankPos);
		// 获取坦克状态，速度
		...
		// 计算坦克与目标之间的距离
		float fBuffer[3] = {0.0}, fBuffer_back[3] = {0.0}, fTargetPos[3] = {0.0};
		GetClientAbsOrigin(iNearestTarget_NotIncap, fTargetPos);
		fBuffer = UpdatePosition(tank, iNearestTarget_NotIncap, g_fTankBhopSpeed);
		fBuffer_back = UpdatePosition(tank, iNearestTarget_NotIncap, -g_fTankBhopSpeed);
		// 是否允许投掷石块？
		if (g_bTankThrow)
		{
		...
		{
			if (iSurvivorDistance <= g_iTankAttackRange && (buttons & IN_ATTACK))
			{
				buttons |= IN_JUMP;
			}
			if (buttons & IN_ATTACK2)
			{
				g_fRockThrowTime[tank] = GetGameTime();
			}
			if (bHasSight && bCanLockVision && !g_bCanTankConsume[tank] && g_bCanTankAttack[tank] && IsSurvivor(iNearestTarget) && (GetGameTime() - g_fRockThrowTime[tank] > (TANK_ROCK_AIM_TIME)))
			{
			...
			if (g_bTankBhop)
			{
				GetClientEyeAngles(tank, fclientEyeAngles);
				if (g_iTankBhopMinRange < iSurvivorDistance && g_iTankBhopMaxRange > iSurvivorDistance && fCurrentSpeed > 200.0)
				{
					if ((iFlags & FL_ONGROUND && !BhopWillHitWall(tank, g_fTankBhopHitWallDistance)))
					{
						...
						if(buttons & IN_BACK)
						{
							Client_Push(tank, fclientEyeAngles, -g_fTankBhopSpeed, VelocityOverride:{VelocityOvr_None,VelocityOvr_None,VelocityOvr_None});
						}
		...
		else
		{
			if (g_bCanTankConsume[tank] && !g_bCanTankAttack[tank] && !g_bInConsumePlace[tank] && bHasLeftRoundPoint(g_fConsumePosition[tank], fTankPos, g_iTankConsumeRaidus * 3))
			{
				...
				if (g_bCanTankConsume[tank] && !g_bInConsumePlace[tank])
				{
					if (g_bTankBhop)
					{
						if (iFlags & FL_ONGROUND && fCurrentSpeed > 220.0 && !BhopWillHitWall(tank, g_fTankBhopHitWallDistance))
						{
							buttons |= IN_DUCK;
							buttons |= IN_JUMP;
							if(buttons & IN_BACK)
...
void TankActionReset(int client)
{
	if (IsAiTank(client))
	{
		...
		SetConVarInt(FindConVar("tank_throw_min_interval"), 6);
...
// 有目的性选择最近非倒地生还
int GetClosestSurvivor_NotIncap(float refpos[3], int excludeSur = -1)
{
	float surPos[3];	int closetSur = GetRandomSurvivor();
	if (closetSur == 0)
	{
...
public Action checkLadderAroundHandler(Handle timer, int client)
{
	...
	for (int i = 0; i < ladderList.Length; i++)
	{
		...
		if (GetVectorDistance(tankPos, curLadderPos) <= 100.0)
		{
			#if (DEBUG_LADDER)
			{
			...
			#endif
			bCanLockVision = false;
			fLockVisonTime = GetGameTime();
			return Plugin_Continue;
		}
	}
	if (GetGameTime() - fLockVisonTime > VISION_UNLOCK_TIME) { bCanLockVision = true; }
	return Plugin_Continue;
}