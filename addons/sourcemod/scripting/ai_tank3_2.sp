...
public void OnPluginStart() {
	...
	g_cvBhopMinDist = CreateConVar("ai_tank_bhop_min_dist", "100", "停止连跳的最小距离", CVAR_FLAGS, true, 0.0);
	g_cvBhopMaxDist = CreateConVar("ai_tank_bhop_max_dist", "99999", "开始连跳的最大距离", CVAR_FLAGS, true, 0.0);
	// when tank's speed is higher than 'ai_tank_bhop_min_speed', he is allowed to bhop, and his max bhop speed will not above 'ai_tank3_bhop_max_speed'
	g_cvBhopMinSpeed = CreateConVar("ai_tank_bhop_min_speed", "200", "连跳的最小速度", CVAR_FLAGS, true, 0.0);
	g_cvBhopMaxSpeed = CreateConVar("ai_tank_bhop_max_speed", "99999", "连跳的最大速度", CVAR_FLAGS, true, 0.0);
	// used to control the speed acceleration during tank each jump from the ground
	g_cvBhopImpulse = CreateConVar("ai_tank_bhop_impulse", "80", "连跳的加速度", CVAR_FLAGS, true, 0.0);
	// allow tank to bhop when he has no sight to any survivor
	g_cvBhopNoVision = CreateConVar("ai_tank_bhop_no_vision", "1", "是否允许坦克在无生还者视野时连跳", CVAR_FLAGS, true, 0.0, true, 1.0);
	...
	g_cvBhopNoVisionMaxAng = CreateConVar("_ai_tank_bhop_nvis_maxang", "180.0", "无生还者视野时速度向量与视角前向向量在这个角度范围内, 允许连跳", CVAR_FLAGS, true, 0.0);
	...
	g_cvAirVecModifyDegree = CreateConVar("ai_tank_airvec_modify_degree", "180.0", "在空中速度方向与自身到目标方向角度超过这个值进行速度修正", CVAR_FLAGS, true, 0.0);
	g_cvAirVecModifyMaxDegree = CreateConVar("ai_tank_airvec_modify_degree_max", "0.0", "在空中速度方向与自身到目标方向角度超过这个值不进行速度修正", CVAR_FLAGS, true, 0.0);
	g_cvAirVecModifyInterval = CreateConVar("ai_tank_airvec_modify_interval", "0.3", "空中速度修正间隔", CVAR_FLAGS, true, 0.1);
	...
	g_cvThrowMinDist = CreateConVar("ai_tank_throw_min_dist", "250", "允许扔石头的最小距离(小于这个距离不允许扔)", CVAR_FLAGS, true, 0.0);
	g_cvThrowMaxDist = CreateConVar("ai_tank_throw_max_dist", "99999", "允许扔石头的最大距离(大于这个距离不允许扔)", CVAR_FLAGS, true, 0.0);
	...
	g_cvClimbAnimRate = CreateConVar("ai_tank_climb_anim_rate", "3.0", "Tank攀爬动画播放速率(是否加速攀爬动作, 1.0=正常倍速)", CVAR_FLAGS, true, 0.0);
	...
	g_cvRockTargetAdjust = CreateConVar("ai_tank_rock_target_adjust", "1", "扔石头时目标不可见是否允许切换目标", CVAR_FLAGS, true, 0.0, true, 1.0);
	g_cvThrowForce = CreateConVar("ai_tank_throw_force", "1000", "扔石头力度", CVAR_FLAGS, true, 0.0);
	// allow tank to punch survivor who is behind him?
	g_cvBackFist = CreateConVar("ai_tank_back_fist", "1", "是否允许Tank使用通背拳(在背后的人也会被拍)", CVAR_FLAGS, true, 0.0, true, 1.0);
	// allow tank to punch survivor who is behind him and within this range (set to -1 to use default: tank_swing_range)
	g_cvBackFistRange = CreateConVar("ai_tank_back_fist_range", "90", "允许使用通背拳时背后的打击检测距离, -1 使用默认(tank_swing_range)", CVAR_FLAGS, true, -1.0);
	// allow tank to lock his vision to his target when punching?
	g_cvPunchLockVision = CreateConVar("ai_tank_punch_lock_vision", "1", "是否允许Tank打拳时锁定视角到目标", CVAR_FLAGS, true, 0.0, true, 1.0);
...
public void L4D_TankClaw_DoSwing_Post(int tank, int claw) {
	...
	GetEntPropVector(tank, Prop_Data, "m_vecVelocity", velVec);
...
public Action L4D_OnCThrowActivate(int ability)
{
	...
	static float velVec[3], vel;
	GetEntPropVector(tankclient, Prop_Data, "m_vecVelocity", velVec);
	vel = SquareRoot(Pow(velVec[0], 2.0) + Pow(velVec[1], 2.0) + Pow(velVec[2], 2.0));
	if (isAiTank(tankclient) && (!g_cvTankBhop.BoolValue || vel < 50) && GetEntProp(tankclient, Prop_Send, "m_zombieClass") == ZC_TANK)
	{
