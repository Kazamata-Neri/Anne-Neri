...
static char TankAndWitch[512];
static char g_sTankPercentText[16];
static char g_sWitchPercentText[16];
static int g_iFailCount;
ConVar hud_style, g_hVsBossBuffer;
KillData g_eData;
Handle g_hTimer;
bool tankInPlay, b_RedHud;
...
{
	hud_style = CreateConVar("l4d2_hud_style", "4", "hud样式切换", _, true, 0.0, true, 4.0);
...
{
	GetTankPercentText(g_sTankPercentText, sizeof(g_sTankPercentText));
	GetWitchPercentText(g_sWitchPercentText, sizeof(g_sWitchPercentText));

	Format(TankAndWitch, sizeof(TankAndWitch), "Tank: [%s]%sWitch: [%s]", g_sTankPercentText, GetAddSpacesMax(5, " "), g_sWitchPercentText);

	if(b_RedHud)
		HUDSetLayout(HUD_SCORE_1, HUD_FLAG_TEXT|HUD_FLAG_NOBG|HUD_FLAG_ALIGN_LEFT|HUD_FLAG_BLINK, "Current: [%d%%]%s%s", GetCurDistance(), GetAddSpacesMax(5, " "), TankAndWitch);
	else
		HUDSetLayout(HUD_SCORE_1, HUD_FLAG_TEXT|HUD_FLAG_NOBG|HUD_FLAG_ALIGN_LEFT, "Current: [%d%%]%s%s", GetCurDistance(), GetAddSpacesMax(5, " "), TankAndWitch);
...
//获取坦克路程
void GetTankPercentText(char[] output, int size)
{
	bool IsStaticTank = false;
	ConVar cv;
	...
	{
		FormatEx(output, size, "%d%%", g_fTankPercent);
	}
	if(IsStaticTank)
	{
		FormatEx(output, size, "固定");
	}
}
...
//获取女巫路程
void GetWitchPercentText(char[] output, int size)
{
	bool IsStaticWitch = false;
	ConVar cv;
	if(g_bWitchAndTankSystemAvailable)
	{
	...
	{
		FormatEx(output, size, "%d%%", g_fWitchPercent);
	}
	if(IsStaticWitch)
	{
		FormatEx(output, size, "固定", output);
	}
}
