...
{
	g_hPlayBackRate = CreateConVar("ai_TankSequencePlayBackRate", "3", "坦克攀爬动画加速速率", FCVAR_NOTIFY, true, 0.0);
...
public void L4D_TankClaw_DoSwing_Pre(int tank, int claw)
{
	...
	{
		SetConVarString(FindConVar("z_tank_throw_force"), "800");
	}
}
		...
		if (DelayExpired(client, 4, TANKROCKAIMDELAY) && !DelayExpired(client, 3, TANKROCKAIMTIME))
		{
			...
			else
			{
				...
				if (IsValidSurvivor(targetclient))
				{
					...
					// 距离小于 100，则说明离生还较近，直接瞄准生还下部即可
					if (dist <= 100)
					{
						// PrintToConsoleAll("[Ai-Tank]：克与最近生还距离小于 100");
						ComputeAimAngles(client, targetclient, aimangles, AimBody);
						aimangles[0] += 20.0;
					}
					//e砖
					else if ((dist / 1000) == 0 && sequence == THROW_OVERHEAD)
					{
						ComputeAimAngles(client, targetclient, aimangles, AimEye);
						aimangles[0] -= dist / PLAYER_HEIGHT * 1.15;
						aimangles[1] += 2.5;
						TeleportEntity(client, NULL_VECTOR, aimangles, NULL_VECTOR);
						return Plugin_Changed;
					}
