...
public void OnPluginStart()
{
	...
	g_hStraightPounceDistance = CreateConVar("ai_hunter_straight_pounce_distance", "90.0", "hunter 允许直扑的范围", CVAR_FLAG, true, 0.0);
	g_hAimOffset = CreateConVar("ai_hunter_aim_offset", "180.0", "与目标水平角度在这一范围内且在直扑范围外，ht 不会直扑", CVAR_FLAG, true, 0.0, true, 360.0);
	g_hNoSightPounceRange = CreateConVar("ai_hunter_no_sign_pounce_range", "1000,250", "hunter 不可见目标时允许飞扑的范围（水平，垂直，逗号分隔，0,0 | x,0 | 0,x=禁用 0 的部分）", CVAR_FLAG);
	g_hBackVision = CreateConVar("ai_hunter_back_vision", "0", "hunter 处在空中时视角背对生还者的概率，0=禁用", CVAR_FLAG, true, 0.0, true, 100.0);
	g_hMeleeFirst = CreateConVar("ai_hunter_melee_first", "1", "hunter 每次准备突袭时是否先按右键（最小最大距离，逗号分隔，0=禁用）");
...
public void OnAllPluginsLoaded()
{
	...
	g_hPounceReadyRange.SetFloat(ignoreCrouch ? 0.0 : 1200.0);
}
...
public Action OnPlayerRunCmd(int hunter, int& buttons, int& impulse, float vel[3], float angles[3], int& weapon, int& subtype, int& cmdnum, int& tickcount, int& seed, int mouse[2])
{
	...
	GetEntPropVector(hunter, Prop_Data, "m_vecVelocity", vecSpeed);
	...
	if (!hasSight && IsValidSurvivor(target))
	{
		...
		if (g_hMeleeFirst.BoolValue &&
			((gametime > timestamp - 0.05) && (gametime < timestamp))/* &&
...
void angleLunge(int hunter, int target, int lungeEntity, float turnAngle)
{
	...
	// hunter 和目标都有效
	if (isValidHunter(hunter) && IsValidSurvivor(target) && IsPlayerAlive(target) && g_hNoSightRevise.BoolValue)
	{
		static float
			selfPos[3],
			targetPos[3];
