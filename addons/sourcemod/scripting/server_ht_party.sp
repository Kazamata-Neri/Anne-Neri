...
{
	SetConVarString(FindConVar("mp_gamemode"), "mutation16");
	return Plugin_Handled;
}
...
public void Event_PlayerHurt(Event event, const char[] name, bool dont_broadcast)
{
	...
	int AddDamage = 0;
	if (g_hSurvivorLimit.IntValue == 1 && IsValidSurvivor(attacker) && IsInfectedBot(victim) && GetEntProp(victim, Prop_Send, "m_zombieClass") == 1)
	{
