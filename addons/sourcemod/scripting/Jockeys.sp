...
public void OnPluginStart()
{
	...
	g_hSpawnDistanceMax = CreateConVar("inf_SpawnDistanceMax", "800.0", "特感复活离生还者最远的距离限制", CVAR_FLAG, true, g_hSpawnDistanceMin.FloatValue);
	g_hTeleportSi = CreateConVar("inf_TeleportSi", "0", "是否开启特感距离生还者一定距离将其传送至生还者周围", CVAR_FLAG, true, 0.0, true, 1.0);
	g_hTeleportDistance = CreateConVar("inf_TeleportDistance", "450.0", "特感落后于最近的生还者超过这个距离则将它们传送", CVAR_FLAG, true, 0.0);
...
//	g_hSpawnMax = CreateConVar("spawn_count_max", "1", "此值记录特感找位次数，根据此值动态改变刷新距离", ~ CVAR_FLAG, true, 0.0);
...
public Action SpawnFirstInfected(Handle timer)
{
	if (!g_bIsLate)
	{
		...
		Handle aSpawnTimer = CreateTimer(g_fSiInterval + 1.0, SpawnNewInfected, _, TIMER_REPEAT);
