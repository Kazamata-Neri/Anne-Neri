...
public Action OnPlayerRunCmd(int smoker, int &buttons, int &impulse, float vel[3], float angles[3], int &weapon)
{
	...
	if (IsAiSmoker(smoker))
	{
		...
		float fSmokerPos[3] = {0.0}, fTargetPos[3] = {0.0}, fTargetAngles[3] = {0.0}, fclientEyeAngles[3] = {0.0};
		GetClientAbsOrigin(smoker, fSmokerPos);
		GetClientEyeAngles(smoker, fclientEyeAngles);
		...
		if (IsSurvivor(iTarget))
		{
			...
			if (g_bSmokerBhop && IsValidSurvivor(iTarget))
			{
				...
				if ((GetEntityFlags(smoker) & FL_ONGROUND) && fCurrentSpeed > 180.0)
				{	
					buttons |= IN_DUCK;
					buttons |= IN_JUMP;
					if(buttons & IN_BACK)
					{
						Client_Push(smoker, fclientEyeAngles, -g_hSmokerBhopSpeed.FloatValue, VelocityOverride:{VelocityOvr_None,VelocityOvr_None,VelocityOvr_None});
					}
